...
if (
  ...
  !commuteScoring.includes("isCommuteTimePenaltyEligible") ||
  !commuteScoring.includes("자동차로 통근하기에 적합한 생활권이 없습니다") ||
  !commuteScoring.includes("대중교통으로 통근하기에 적합한 생활권이 없습니다")
) {
  throw new Error("commuteScoring.js must apply actual-API-only medium/high commute overrun penalties and mode-specific no-result states.");
}
